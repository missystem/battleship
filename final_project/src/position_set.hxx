...
/// `coord_limit` below. Right now it's 20, which means that the largest
/// position a set can hold is {9, 19}.
class Position_set
{
public:
    /// All position coordinates must be between 0 (inclusive) and
    /// `coord_limit` (exclusive).
    static constexpr int coord_limit = 20;
...
private:
    ...
    static constexpr std::size_t index_limit = 10 * coord_limit;
